...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background, reconstruct=False):
    ...
    for i in range(test_times):
        for idx, (gt_img, view) in enumerate(tqdm(views, desc="Rendering progress")):
            if idx == 0 and i == 0:
                time1 = time()
...
def render_sets(dataset: ModelParams, opt, iteration: int, pipeline: PipelineParams, checkpoint,
                ...
                num_pts, num_pts_ratio, batch_size=1):
    with torch.no_grad():
...
if __name__ == "__main__":
    ...
    render_sets(lp.extract(args), op.extract(args), args.iteration, pp.extract(args), args.checkpoint, args.skip_train,
                ...
                args.num_pts, args.num_pts_ratio, args.batch_size)