...
class GaussianModel:
    ...
    def subset_view(self, indices: torch.Tensor):
        """Return a lightweight view of this model restricted to a subset of Gaussians.
        This does not modify the original model; it returns a shallow-copied structure with tensor views.
        """
        if not torch.is_tensor(indices):
            indices = torch.as_tensor(indices, device=self._xyz.device, dtype=torch.long)
        else:
            indices = indices.to(device=self._xyz.device, dtype=torch.long)

        new = GaussianModel(self.max_sh_degree, gaussian_dim=self.gaussian_dim, time_duration=self.time_duration, rot_4d=self.rot_4d, force_sh_3d=self.force_sh_3d, sh_degree_t=self.max_sh_degree_t)
        # Copy simple attributes
        new.active_sh_degree = self.active_sh_degree
        new.active_sh_degree_t = self.active_sh_degree_t
        new.spatial_lr_scale = self.spatial_lr_scale
        new.percent_dense = self.percent_dense
        new.env_map = self.env_map

        # Slice tensor storages as views
        new._xyz = self._xyz[indices]
        new._t = self._t[indices] if self.gaussian_dim == 4 and self._t.numel() > 0 else self._t
        new._features_dc = self._features_dc[indices]
        new._features_rest = self._features_rest[indices]
        new._scaling = self._scaling[indices]
        new._opacity = self._opacity[indices]
        new._rotation = self._rotation[indices]
        if self.gaussian_dim == 4:
            new._scaling_t = self._scaling_t[indices]
            new._rotation_r = self._rotation_r[indices]

        # No optimizer for inference subset view
        new.optimizer = None
        new.max_radii2D = None
        new.xyz_gradient_accum = None
        new.t_gradient_accum = None
        new.denom = None
        return new
